...
export default function FormBuilderEnhanced({ existingForm, onSaveComplete, companyId }: FormBuilderEnhancedProps = {}) {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <Card
        className={cn(
          "relative overflow-hidden rounded-xl border border-[#1a1a1a]/70 bg-gradient-to-br from-[#0f0f0f] via-[#1a1a1a] to-[#0f0f0f]",
          "shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_2px_12px_rgba(0,0,0,0.6)]"
        )}
      >
        {/* metallic sheen */}
        ...
        <CardContent className="relative p-6 sm:p-8 space-y-8">
          ...
          <section className="space-y-3">
            <div className="grid gap-3">
              <label className="text-sm text-[#F8FAFC]">Form Name <span className="text-emerald-400">*</span></label>
              <input
                value={draft.name}
                onChange={(e) => setDraft((d) => ({ ...d, name: e.target.value }))}
                placeholder="e.g., Course Feedback Survey"
                className={cn(
                  "h-11 w-full rounded-xl bg-[#0F1319] px-3 text-sm text-zinc-200 placeholder:text-[#A1A1AA]",
                  "border border-[#2A2F36] focus:border-emerald-600 focus:outline-none"
                )}
              />
            </div>
            <div className="grid gap-3">
              <label className="text-sm text-[#F8FAFC]">Description</label>
              <textarea
                ...
                rows={3}
                className={cn(
                  "w-full resize-y rounded-xl bg-[#0F1319] p-3 text-sm text-zinc-200 placeholder:text-[#A1A1AA]",
                  "border border-[#2A2F36] focus:border-emerald-600 focus:outline-none"
                )}
              />
            </div>
          ...
          <section className="space-y-3">
            ...
            {draft.fields.length === 0 ? (
              <p className="text-sm text-[#A1A1AA]">No fields added yet. Add your first field below.</p>
            ) : (
              <ul className="space-y-3">
                {draft.fields.map((f, i) => (
                  <li key={f.id} className="rounded-xl border border-[#1a1a1a]/70 bg-[#0a0a0a] p-3">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div className="space-y-1">
                        ...
                        {f.placeholder && (
                          <div className="text-xs text-[#A1A1AA]">Placeholder: {f.placeholder}</div>
                        )}
                        ...
                        {f.type === "rating" && (
                          <div className="text-xs text-[#A1A1AA]">Max: {f.max ?? DEFAULT_RATING_MAX}</div>
                        )}
                      </div>
                    ...
                    <div className="mt-3 grid gap-2 sm:grid-cols-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="text-sm text-[#F8FAFC]">Required</span>
                      </div>
                      <input
                        value={f.placeholder || ""}
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {SURVEY_PRESETS.map((preset) => (
                  <Card key={preset.id} className="border border-[#1a1a1a]/70 bg-gradient-to-br from-[#0f0f0f] via-[#1a1a1a] to-[#0f0f0f] hover:border-[#10B981]/30 transition-colors">
          ...
          {showPreview && (
            <section className="space-y-4">
              ...
              <Card className="border border-[#1a1a1a]/70 bg-gradient-to-br from-[#0f0f0f] via-[#1a1a1a] to-[#0f0f0f]">
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-[1fr,240px]">
              ...
              <div className="grid gap-2">
                <label className="text-xs text-[#A1A1AA]">Field Type</label>
                <select
                  value={type}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="grid gap-2">
                <label className="text-xs text-[#A1A1AA]">Placeholder (optional)</label>
                <input
                  value={placeholder}
              ...
              <label className="mt-7 flex items-center gap-2">
                <input
                  ...
                  className="h-4 w-4 accent-emerald-600"
                />
                <span className="text-sm text-[#F8FAFC]">Required field</span>
              </label>
            </div>

            {/* Options builder */}
            {needsOptions && (
              <div className="grid gap-2 rounded-xl border border-[#1a1a1a]/70 bg-[#0a0a0a] p-3">
                <div className="text-xs font-medium text-[#F8FAFC]">Options</div>
                <div className="flex gap-2">
                  <input
                    value={newOption}
                ...
                {options.length > 0 && (
                  <ul className="flex flex-wrap gap-2">
                    {options.map((opt, i) => (
                      <li key={i} className="flex items-center gap-2 rounded-lg border border-[#1a1a1a]/70 bg-[#0a0a0a] px-2 py-1 text-xs text-[#F8FAFC]">
                        {opt}
                        <button
            ...
            {needsRating && (
              <div className="grid gap-2 sm:max-w-xs">
                <label className="text-xs text-[#A1A1AA]">Max rating</label>
                <input
                  type="number"
                  min={2}
                  max={10}
